...
for x0, y0 in initial_conditions:
    ...
    plt.plot(t_values, x_values, label='x', (x0, y0), linewidth=2)
    plt.plot(t_values, y_values, label='y', (x0, y0), linewidth=2)
...
for x0, y0 in initial_conditions:
    t_values, x_values, y_values = runge_kutta_4(lotka_volterra, x0, y0, T, dt)
    plt.plot(x_values, y_values, linewidth=2, label=f'({x0}, {y0})')
    plt.plot(x_values[0], y_values[0], marker='o', markersize=5)

plt.xlabel("x")
plt.ylabel("y")
plt.title("Phase portrait of Lotka-Volter model")
plt.legend()
plt.show()
